...
function Navbar() {
  ...
  const mainFramePath: Paths = [{
    ...
    path: [["M", sideOffset, `100% - 12`], ["L", `${topOffset + topCorner}`, `${topCorner + topInset}`], ["Q", topOffset, `${topInset}`, `${topOffset + topCorner * 2}`, `${topInset}`], ["L", `100% - ${topOffset + topCorner * 2}`, `${topInset}`], ["Q", `100% - ${topOffset}`, `${topInset}`, `100% - ${topOffset + topCorner}`, `${topCorner + topInset}`], ["L", `100% - ${sideOffset}`, `100% - 12`], ["L", `50% + ${halfTab + 30}`, `100% - 12`], ["L", `50% + ${halfTab}`, `100% + 14`], ["L", `50% - ${halfTab}`, `100% + 14`], ["L", `50% - ${halfTab + 30}`, `100% - 12`], ["Z"]]
  }];
  return <MobileMenuContext.Provider value={{
    ...
    setShowMenu
      ...
      <nav className="fixed left-0 right-0 top-0 z-[100] px-4 lg:px-8 pt-2 lg:pt-3 scale-[0.88] origin-top">
        <div ref={navRef} className="h-11 mt-1 mx-2 lg:-mt-1 lg:-mx-px w-full relative top-0 inset-x-0 z-40">
          ...
          <div className="absolute inset-0 w-full h-full pointer-events-none z-20">
            ...
            path: [["M", ledStartX, "100%"], ["L", `50% - ${halfTab + 35}`, "100%"], ["L", `50% - ${halfTab}`, "100% + 26"]]
          }]} />
          </div>
          ...
          <div className="absolute inset-0 w-full h-full pointer-events-none z-20">
            ...
            path: [["M", ledEndX, "100%"], ["L", `50% + ${halfTab + 35}`, "100%"], ["L", `50% + ${halfTab}`, "100% + 26"]]
          }]} />
          </div>
          ...
          <div className="absolute inset-0 w-full h-full flex items-center justify-center z-30">
            <div ref={contentRef} className="flex-none flex items-center px-8 mt-4">
              ...
              <div className="hidden lg:flex gap-10 font-medium text-base">
                {navLinks.map(link => <a key={link.label} href={link.href} onClick={e => handleClick(e, link.href)} className="text-white/90 hover:text-white transition-colors tracking-[0.2em] font-semibold">
                    {link.label}
                  </a>)}
              </div>
              {/* Mobile: mostra Menu centralizado com ícone */}
              <div onClick={() => setShowMenu(true)} className="cursor-pointer flex items-center gap-2 lg:hidden font-medium text-white">
      ...
      {showMenu && <div className="fixed inset-0 z-[110] lg:hidden">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={() => setShowMenu(false)} />
          <div className="absolute left-4 right-4 top-24">
            <div className="relative overflow-hidden rounded-lg p-6 backdrop-blur-xl" style={{
          ...
          boxShadow: "0 0 30px rgba(0,0,0,0.5)"
              ...
              <div className="mb-6 flex items-center justify-between">
                ...
                <button onClick={() => setShowMenu(false)} className="text-white/80 hover:text-white">
                  <X className="h-6 w-6" />
                </button>
              </div>

              <div className="flex flex-col gap-4">
                {navLinks.map(link => <a key={link.label} href={link.href} onClick={e => handleClick(e, link.href)} className="text-lg font-semibold text-white/80 transition-colors hover:text-white tracking-[0.2em]">
                    {link.label}
                  </a>)}
              </div>
            </div>
